...
_ZH_TW: dict[str, str] = {
    ...
    "browser.profile.desc": "本程式的進階功能（純淨視窗、套用視窗位置 / 大小、視窗最小化、不顯示在工作列、直播結束 / 停止 / 離題時自動關窗）都需要這裡。代價是與你主瀏覽器的登入狀態分開，每個 Profile 需要各自登入一次（可用下方「登入此 Profile」協助）。",
    ...
    "browser.btn.signin": "登入此 Profile",
    "browser.btn.save": "儲存",
    ...
    "browser.msg.signin_opened": "已用獨立 Profile 開啟瀏覽器，請完成登入後關閉視窗。Cookies 會留在此 Profile 內。",
    "browser.msg.signin_no_path": "請先啟用「使用獨立 Profile」並填入路徑，再執行登入。",
    ...
    "browser.msg.no_isolation_warning": "目前未啟用獨立 Profile。若你勾選了 純淨視窗 (App Mode) / 視窗最小化 / 不顯示在工作列 / 自動關閉視窗（離線 / 停止 / 離題） 之中任何一項，按「儲存」時系統會自動把獨立 Profile 與每頻道登入環境啟用到預設路徑，讓這些功能可以正常運作。若想自選 Profile 路徑或維持共用瀏覽器，請手動勾選 / 取消「使用獨立 Profile」並填入路徑。",
    "browser.close.title": "儲存瀏覽器設定？",
    "browser.close.body": "在關閉前要儲存你修改的瀏覽器設定嗎？",
    # User-tab banner shown when enabled + opt-in iso flags + no profile path.
    # Doubles as the action-link to switch to the Advanced tab.
    "browser.banner.iso_features_auto_setup": "你勾選的進階選項需要「獨立 Profile」。儲存時會自動啟用到預設路徑——點此跳到「進階」分頁查看 / 自訂。",
    ...
    "browser.msg.migration_applied": "儲存時自動啟用了獨立 Profile（{path}）與每頻道登入環境，這樣你勾選的進階功能才會生效。",
...
_ZH_CN: dict[str, str] = {
    ...
    "browser.profile.desc": "本程序的进阶功能（纯净窗口、套用窗口位置 / 大小、窗口最小化、不显示在任务栏、直播结束 / 停止 / 离题时自动关窗）都需要这里。代价是与你主浏览器的登录状态分开，每个 Profile 需要各自登录一次（可用下方「登录此 Profile」协助）。",
    ...
    "browser.msg.no_isolation_warning": "目前未启用独立 Profile。若你勾选了 纯净窗口 (App Mode) / 窗口最小化 / 不显示在任务栏 / 自动关闭窗口（离线 / 停止 / 离题） 之中任何一项，按「保存」时系统会自动把独立 Profile 与每频道登录环境启用到默认路径，让这些功能可以正常运作。若想自选 Profile 路径或维持共用浏览器，请手动勾选 / 取消「使用独立 Profile」并填入路径。",
    "browser.close.title": "保存浏览器设置？",
    "browser.close.body": "在关闭前要保存你修改的浏览器设置吗？",
    "browser.banner.iso_features_auto_setup": "你勾选的进阶选项需要「独立 Profile」。保存时会自动启用到默认路径——点此跳到「进阶」分页查看 / 自订。",
    ...
    "browser.msg.migration_applied": "保存时自动启用了独立 Profile（{path}）与每频道登录环境，这样你勾选的进阶功能才会生效。",
...
_EN: dict[str, str] = {
    ...
    "browser.profile.desc": "Required by every advanced feature (clean window, apply window position / size, minimize on launch, hide from taskbar, auto-close on offline / stop / off-topic). Trade-off: the profile is separate from your main browser, so each profile needs its own sign-in (use the \"Sign in to this profile\" button below to bootstrap).",
    ...
    "browser.msg.no_isolation_warning": "No dedicated profile is set up yet. If any of Clean Window (App Mode) / Minimize on launch / Hide from taskbar / Auto-close window (offline / stop / off-topic) are checked, clicking Save will automatically enable the dedicated profile and per-channel sub-profiles at the default path so those features can actually run. To pick a custom profile path — or to stay on the shared browser — tick / untick \"Use a dedicated profile\" and edit the path manually.",
    ...
    "browser.banner.iso_features_auto_setup": "The advanced options you ticked need a dedicated profile. Saving will set one up at the default path — click here to jump to the Advanced tab to review or customise.",
    ...
    "browser.msg.migration_applied": "Saving auto-enabled the dedicated profile ({path}) and per-channel sub-profiles so the advanced options you ticked can actually take effect.",
...
_JA: dict[str, str] = {
    ...
    "browser.profile.desc": "本アプリの高度機能（クリーンウィンドウ、ウィンドウ位置 / サイズ適用、起動時最小化、タスクバー非表示、配信終了 / 停止 / 無関係時の自動クローズ）はすべてこの設定に依存します。メインブラウザのログイン状態とは分かれるため、各プロファイルごとに一度ログインが必要です（下の「このプロファイルでログイン」ボタンを利用）。",
    ...
    "browser.msg.no_isolation_warning": "独立プロファイルがまだ設定されていません。クリーンウィンドウ (App Mode) / 起動時に最小化 / タスクバーに表示しない / 自動でウィンドウを閉じる（オフライン / 停止 / 無関係）のいずれかをチェックしている場合、「保存」を押すと既定パスに独立プロファイルとチャンネル別ログイン環境が自動で有効化され、これらの機能が動作します。プロファイルパスをカスタムしたい、もしくは共有ブラウザのままにしたい場合は「独立プロファイルを使用」を手動で切り替えてパスを編集してください。",
    "browser.close.title": "ブラウザ設定を保存しますか？",
    "browser.close.body": "閉じる前に、ブラウザ設定の変更を保存しますか？",
    "browser.banner.iso_features_auto_setup": "チェックした高度オプションには「独立プロファイル」が必要です。保存時に既定パスへ自動設定されます — クリックして「高度設定」タブで確認 / カスタマイズしてください。",
    ...
    "browser.msg.migration_applied": "保存時に独立プロファイル（{path}）とチャンネル別ログイン環境を自動有効化しました。これでチェックした高度機能が動作します。",
...
_KO: dict[str, str] = {
    ...
    "browser.profile.desc": "본 프로그램의 고급 기능(깔끔한 창, 창 위치 / 크기 적용, 시작 시 최소화, 작업 표시줄에서 숨기기, 방송 종료 / 중지 / 주제 외일 때 자동 닫기)은 모두 여기를 필요로 합니다. 메인 브라우저의 로그인 상태와는 분리되므로 각 프로필마다 한 번씩 로그인해야 합니다(아래 「이 프로필에 로그인」 버튼 활용).",
    ...
    "browser.msg.no_isolation_warning": "아직 독립 프로필이 설정되어 있지 않습니다. 깔끔한 창 (App Mode) / 시작 시 최소화 / 작업 표시줄에서 숨기기 / 창 자동 닫기(오프라인 / 중지 / 주제 외) 중 하나라도 체크되어 있다면 「저장」을 누를 때 기본 경로에 독립 프로필과 채널별 로그인 환경이 자동으로 활성화되어 이 기능들이 정상 작동합니다. 프로필 경로를 직접 지정하거나 공유 브라우저로 유지하려면 「독립 프로필 사용」을 수동으로 체크 / 해제하고 경로를 편집하세요.",
    "browser.close.title": "브라우저 설정을 저장하시겠습니까?",
    "browser.close.body": "닫기 전에 브라우저 설정 변경 사항을 저장하시겠습니까?",
    "browser.banner.iso_features_auto_setup": "체크한 고급 옵션에는 「독립 프로필」이 필요합니다. 저장 시 기본 경로에 자동으로 설정됩니다 — 클릭해서 「고급」 탭에서 확인 / 사용자 지정하세요.",
    ...
    "browser.msg.migration_applied": "저장 시 독립 프로필({path})과 채널별 로그인 환경을 자동으로 활성화했습니다. 이제 체크한 고급 기능이 작동합니다.",
