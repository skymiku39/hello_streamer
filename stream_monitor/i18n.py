...
_ZH_TW: dict[str, str] = {
    ...
    "browser.msg.no_window_tracking_warning": "目前以「分頁」模式開啟（未勾選獨立視窗且未啟用 App Mode）。程式無法登記或關閉該分頁，下方進階選項已停用；請勾選獨立視窗或啟用 App Mode。",
...
_ZH_CN: dict[str, str] = {
    ...
    "browser.msg.no_window_tracking_warning": "目前以「标签页」模式打开（未勾选独立窗口且未启用 App Mode）。程序无法登记或关闭该标签页，下方进阶选项已停用；请勾选独立窗口或启用 App Mode。",
...
_EN: dict[str, str] = {
    ...
    "browser.msg.no_window_tracking_warning": "Tab mode is active (separate window and App Mode are both off). Hello Streamer cannot register or close that tab, so the advanced options below are disabled. Enable App Mode or tick \"Open in a separate window\".",
...
_JA: dict[str, str] = {
    ...
    "browser.msg.no_window_tracking_warning": "現在はタブモードです（独立ウィンドウも App Mode もオフ）。このタブは登録・自動クローズできないため、下の高度なオプションは無効です。独立ウィンドウをオンにするか App Mode を有効にしてください。",
...
_KO: dict[str, str] = {
    ...
    "browser.msg.no_window_tracking_warning": "현재 탭 모드입니다(독립 창과 App Mode가 모두 꺼짐). 이 탭은 등록하거나 자동으로 닫을 수 없어 아래 고급 옵션이 비활성화됩니다. 독립 창을 켜거나 App Mode를 사용하세요.",
