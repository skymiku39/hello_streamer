...
_ZH_TW: dict[str, str] = {
    ...
    "browser.toggle.close_on_stop": "🛑 停止監聽時自動關閉所有本程式開啟的視窗",
    "browser.toggle.close_on_stop.hint": "點下底部「停止」按鈕時，會 WM_CLOSE 掉所有本程式追蹤中的播放器視窗（不會碰到你原本就開著的瀏覽器分頁）。",
    "browser.toggle.close_off_topic": "🎯 僅保留直播相關視窗（自動關掉被導離的）",
    "browser.toggle.close_off_topic.hint": "每輪輪詢後檢查視窗標題，若內容變成新分頁、首頁、廣告頁等與直播無關的頁面，且是本程式開啟的視窗，會自動 WM_CLOSE。",
    ...
    "browser.msg.profile_autofix": "偵測到「每頻道獨立子資料夾」已開啟但獨立 Profile 路徑為空 — 已自動帶入預設路徑，請按「儲存」生效。否則所有頻道會共用同一個 Chrome 主程序，自動關閉視窗可能誤殺直播。",
...
_ZH_CN: dict[str, str] = {
    ...
    # Browser settings dialog
    "browser.title": "浏览器设置",
    "browser.section.open": "浏览器打开方式",
    "browser.section.open.hint": "关闭自定义模式时，将使用系统默认浏览器（无法控制坐标／窗口）。",
    "browser.enable": "启用自定义浏览器打开（subprocess 模式）",
    "browser.path.label": "浏览器命令",
    "browser.path.placeholder": "chrome / msedge / 或浏览器可执行文件绝对路径",
    "browser.path.hint": "提示：chrome、msedge 可直接输入；若未在 PATH 内请填完整 .exe 路径。",
    "browser.toggle.new_window": "强制独立新窗口 (--new-window)",
    "browser.toggle.app_mode": "App Mode：纯净播放器 (--app=URL；已隐含独立窗口)",
    "browser.toggle.minimized": "最小化启动（不抢焦点；由本程序在窗口出现后自动 Win32 最小化）",
    "browser.toggle.close_on_offline": "🗙 下播时自动关闭播放器窗口 (PostMessage WM_CLOSE)",
    "browser.toggle.close_on_offline.hint": "仅关闭本程序打开的窗口（HWND 已登记）；找不到时才用窗口标题关键字后备。",
    "browser.toggle.close_on_stop": "🛑 停止监听时自动关闭所有本程序打开的窗口",
    "browser.toggle.close_on_stop.hint": "点击底部「停止」按钮时，会 WM_CLOSE 掉所有本程序追踪中的播放器窗口（不会碰到你原本就打开的浏览器标签页）。",
    "browser.toggle.close_off_topic": "🎯 仅保留直播相关窗口（自动关掉被导离的）",
    "browser.toggle.close_off_topic.hint": "每轮轮询后检查窗口标题，若内容变成新标签页、首页、广告页等与直播无关的页面，且是本程序打开的窗口，会自动 WM_CLOSE。",
    "browser.toggle.hide_taskbar": "👻 从任务栏与 Alt+Tab 隐藏 (WS_EX_TOOLWINDOW)",
    "browser.toggle.hide_taskbar.hint": "窗口依然可见，但不会占任务栏空间，也不会在 Alt+Tab 中干扰。\n代价：无法从任务栏把它叫到最上层（需从本程序控制）。",
    "browser.profile.title": "独立浏览器 Profile（建议启用）",
    "browser.profile.desc": "Chrome / Edge 已打开时，--app= 与坐标/大小会被忽略。设置独立 Profile 可强制浏览器以新 master process 打开，所有设置才会生效。\n代价：此 profile 没有你主浏览器的书签/登录/扩展（弹窗纯粹播放用）。",
    "browser.profile.enable": "启用独立 Profile",
    "browser.profile.per_channel": "每个频道使用独立子文件夹（强烈建议）",
    "browser.profile.per_channel.hint": "开启时：browser_profile/twitch_<channel>、youtube_<handle> 各一份。\n确保 App Mode 纯净窗口对每个频道都有效（不会被 Chrome master process 吃掉），代价是每个频道需重新登录一次。",
    ...
    "browser.btn.save": "保存",
    "browser.compat.disabled": "（自定义模式已停用 — 将使用系统默认浏览器）",
    "browser.compat.firefox": "⚠ 检测到 Firefox：坐标 / 大小 / App Mode 都无法控制（CLI 不支持），相关字段已停用。",
    "browser.compat.chromium": "✓ Chromium 系列浏览器：所有参数都可使用。",
    "browser.compat.unknown": "ℹ 未知浏览器类型 — 仍会尝试发送 Chromium 参数，若无效请改用 chrome / msedge。",
    ...
    "browser.msg.profile_autofix": "检测到「每个频道独立子文件夹」已开启但独立 Profile 路径为空 — 已自动填入默认路径，请按「保存」生效。否则所有频道会共用同一个 Chrome 主进程，自动关闭窗口可能误杀直播。",
    "browser.msg.reset_done": "已恢复窗口默认值 (0, 0, 1280×720)。",
    "browser.msg.test_opened": "已打开本机测试页；App Mode 会使用独立测试 Profile。",
...
_EN: dict[str, str] = {
    ...
    "browser.toggle.close_on_stop": "🛑 Close every player window this app opened when you stop monitoring",
    "browser.toggle.close_on_stop.hint": "Clicking the bottom \"Stop\" button sends WM_CLOSE to every player window we tracked. Your own browser tabs are never touched.",
    "browser.toggle.close_off_topic": "🎯 Keep only stream windows (auto-close redirected windows)",
    "browser.toggle.close_off_topic.hint": "After each poll, inspects window titles and WM_CLOSEs any tracked window that has drifted to a new tab, homepage, ad page, etc.",
    ...
    "browser.msg.profile_autofix": "\"Per-channel sub-folder\" is on but the dedicated profile path was empty — the default has been filled in. Click \"Save\" to apply, otherwise every channel shares one Chrome master process and the auto-close feature may kill live windows by mistake.",
...
_JA: dict[str, str] = {
    ...
    "browser.toggle.close_on_stop": "🛑 監視停止時、このアプリが開いた全ウィンドウを自動で閉じる",
    "browser.toggle.close_on_stop.hint": "下部の「停止」ボタンを押すと、本アプリが追跡中の全プレイヤーウィンドウへ WM_CLOSE を送ります。元から開いていたタブには触れません。",
    "browser.toggle.close_off_topic": "🎯 配信関連ウィンドウのみ保持（リダイレクトされたものを自動で閉じる）",
    "browser.toggle.close_off_topic.hint": "各ポーリング後にウィンドウタイトルを確認し、新しいタブ・ホーム・広告ページなどへ遷移していたら自動で WM_CLOSE します。",
    ...
    "browser.msg.profile_autofix": "「チャンネル別の独立フォルダ」が ON ですが独立プロファイルのパスが空です — デフォルト値を自動入力しました。「保存」を押して有効化してください。そうしないと全チャンネルが同一の Chrome マスタープロセスを共有し、自動クローズ機能が配信ウィンドウを誤って閉じる可能性があります。",
...
_KO: dict[str, str] = {
    ...
    "browser.path.hint": "팁: chrome, msedge는 그대로 입력 가능. PATH에 없으면 .exe 전체 경로를 입력하세요.",
    "browser.toggle.new_window": "독립 새 창 강제 (--new-window)",
    "browser.toggle.app_mode": "App Mode: 깔끔한 플레이어 (--app=URL; 독립 창 포함)",
    "browser.toggle.minimized": "최소화로 시작 (포커스 빼앗지 않음; 창이 뜨면 Win32로 최소화)",
    "browser.toggle.close_on_offline": "🗙 방송 종료 시 플레이어 창 자동 닫기 (PostMessage WM_CLOSE)",
    "browser.toggle.close_on_offline.hint": "이 앱이 연 창(HWND 등록)만 닫습니다. 찾지 못하면 창 제목 키워드로 시도합니다.",
    "browser.toggle.close_on_stop": "🛑 감시 중지 시 이 앱이 연 모든 창 자동 닫기",
    "browser.toggle.close_on_stop.hint": "하단의 \"중지\" 버튼을 누르면 추적 중인 모든 플레이어 창에 WM_CLOSE를 보냅니다. 사용자가 직접 연 탭은 건드리지 않습니다.",
    "browser.toggle.close_off_topic": "🎯 방송 관련 창만 유지 (리다이렉트된 창 자동 닫기)",
    "browser.toggle.close_off_topic.hint": "매 폴링 후 창 제목을 확인하여 새 탭·홈·광고 페이지 등으로 이동했다면 WM_CLOSE합니다.",
    "browser.toggle.hide_taskbar": "👻 작업 표시줄과 Alt+Tab에서 숨기기 (WS_EX_TOOLWINDOW)",
    "browser.toggle.hide_taskbar.hint": "창은 그대로 보이지만 작업 표시줄/Alt+Tab에 나타나지 않습니다.\n트레이드오프: 작업 표시줄에서 앞으로 가져올 수 없습니다 (이 앱에서 제어).",
    "browser.profile.title": "독립 브라우저 프로필 (권장)",
    "browser.profile.desc": "Chrome / Edge가 이미 실행 중이면 --app=와 좌표/크기가 무시됩니다. 독립 프로필을 설정하면 새 master process가 강제 실행되어 모든 설정이 적용됩니다.\n트레이드오프: 이 프로필에는 메인 브라우저의 북마크/로그인/확장이 없습니다 (재생 전용).",
    "browser.profile.enable": "독립 프로필 활성화",
    "browser.profile.per_channel": "채널별로 별도 하위 폴더 사용 (강력 권장)",
    "browser.profile.per_channel.hint": "켜짐: browser_profile/twitch_<channel>, youtube_<handle>가 각각 생성됩니다.\nApp Mode의 깔끔한 창을 모든 채널에 보장 (Chrome master process 충돌 방지). 트레이드오프: 채널별 재로그인 필요.",
    ...
    "browser.compat.unknown": "ℹ 알 수 없는 브라우저 — Chromium 스타일 매개변수를 시도합니다. 실패 시 chrome / msedge를 사용하세요.",
    ...
    "browser.msg.profile_autofix": "「채널별 독립 하위 폴더」가 켜져 있지만 독립 프로필 경로가 비어 있어 기본 경로를 자동으로 채웠습니다. 「저장」을 눌러 적용하세요. 그렇지 않으면 모든 채널이 하나의 Chrome 마스터 프로세스를 공유하게 되어, 자동 창 닫기 기능이 실제 방송 창을 잘못 닫을 수 있습니다.",
    "browser.msg.reset_done": "기본 창으로 재설정됨 (0, 0, 1280×720).",
    "browser.msg.test_opened": "테스트 페이지가 열렸습니다. App Mode는 독립 테스트 프로필을 사용합니다.",
