...
_ZH_TW: dict[str, str] = {
    ...
    "browser.msg.no_isolation_warning": "未設定獨立 Profile：自動關閉、視窗位置 / 大小、純淨視窗都可能不穩定（會誤關或設定不生效）。建議啟用「使用獨立 Profile」。",
...
_ZH_CN: dict[str, str] = {
    ...
    "browser.msg.no_isolation_warning": "未设定独立 Profile：自动关闭、窗口位置 / 大小、纯净窗口都可能不稳定（会误关或设置不生效）。建议启用「使用独立 Profile」。",
...
_EN: dict[str, str] = {
    ...
    "browser.msg.no_isolation_warning": "No dedicated profile is configured: auto-close, window position / size, and clean windows may all behave unreliably (wrong windows closed, settings ignored). Enabling \"Use a dedicated profile\" is recommended.",
...
_JA: dict[str, str] = {
    ...
    "browser.msg.no_isolation_warning": "独立プロファイルが設定されていません：自動クローズ、ウィンドウ位置／サイズ、クリーンウィンドウのいずれも不安定になり得ます（誤クローズや設定無視の恐れあり）。「独立プロファイルを使用」を有効化することを推奨します。",
...
_KO: dict[str, str] = {
    ...
    "browser.msg.no_isolation_warning": "독립 프로필이 설정되지 않았습니다: 자동 닫기, 창 위치 / 크기, 깔끔한 창 모두 불안정할 수 있습니다(잘못 닫히거나 설정이 무시될 수 있음). 「독립 프로필 사용」을 활성화하는 것을 권장합니다.",
