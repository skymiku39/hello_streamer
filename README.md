...
| 平台 | 檔案 |
| --- | --- |
| Windows x64 | `HelloStreamer-v0.8.0-windows-x64.exe` |
| Linux x64 | `HelloStreamer-v0.8.0-linux-x64.tar.gz` |
| Linux ARM64 / Raspberry Pi 64-bit | `HelloStreamer-v0.8.0-linux-arm64.tar.gz` |
...
| 設定 | 說明 |
| --- | --- |
| 純淨播放器視窗 | 使用 Chromium 的 `--app=URL`，開出沒有網址列的播放器視窗。勾選後會自動啟用並鎖定獨立視窗。 |
| 獨立 Profile | 使用獨立 `user-data-dir`，避免既有 Chrome / Edge 主程序吃掉 `--app`、視窗座標與大小參數。可用「登入此 Profile」先完成 Twitch / YouTube 登入。 |
