...
| 平台 | 檔案 |
| --- | --- |
| Windows x64 | `HelloStreamer-v0.9.11-windows-x64.exe` |
| Linux x64 | `HelloStreamer-v0.9.11-linux-x64.tar.gz` |
| Linux ARM64 / Raspberry Pi 64-bit | `HelloStreamer-v0.9.11-linux-arm64.tar.gz` |
...
| log 關鍵字 | 意義 |
| --- | --- |
| `post_resume_grace` | 偵測到長時間暫停（如睡眠）後的第一輪輪詢；本輪 `fetch returned None` / `fetch exception` **不計入**離線防抖，避免假下播。 |
| `Monitor thread died unexpectedly` | 背景監控執行緒異常結束；程式會嘗試自動重啟，底部狀態短暫顯示「監控已自動恢復…」。 |
...
若啟用「直播結束時關閉播放器視窗」（`close_on_offline`），喚醒後連續兩輪網路失敗仍可能誤關播放器；睡眠後的第一輪已有 grace 保護，第二輪起請確認網路已恢復。
