...
- **Trello 式拖曳**：頻道列 ▲▼ 之間新增把手，長按後可拖曳或滾輪調整順序。
- **推擠預覽**：拖曳時鄰近列即時讓位；放開後寫入 `config.json` 並同步監控順序。
- **直覺操作**：需實際移動或滾輪才會改變順序；拖回原位可取消；支援 Esc 放棄。
- **座標穩定**：canvas 內容座標對齊，邊拖曳邊捲動時插入點不再偏移；滾輪調序不會誤捲動外層列表。
- **流暢重排**：`after(0)` 即時重排與幾何鎖定，減少預覽抖動。
- **操作保護**：拖曳中刪除頻道會先安全結束排序；修正把手外放開時狀態殘留。
